...
COPY --from=compile-stage --chown=root:node --chmod=755 /cache/.npm /cache/.npm

RUN npm ci --omit=dev --cache /cache/.npm && \
    rm -rf package-lock.json /cache/.npm

COPY --chown=root:node --chmod=755 public ./public
COPY --from=compile-stage --chown=root:node --chmod=755 /tmp/app/.next ./.next
COPY --chown=root:node --chmod=755 next.config.mjs ./
