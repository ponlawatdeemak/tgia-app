				...
				<div className='flex flex-col gap-1'>
					<div className='mx-4 mb-2 mt-4 flex flex-row items-center gap-3 lg:mx-6'>
						<Button
							...
							onClick={() => router.push(AppPath.PlotMonitoringResult)}
						>
					...
					<div className='mx-6 mb-2 mt-3 flex text-base font-semibold text-black max-lg:hidden'>
						{`${t('occurrence', { ns: 'plot-monitoring' })} ${plantDetailData?.count} ${plantDetailData?.lossType === 'noData' || plantDetailData?.lossType === 'noDamage' ? t('noDisaster') : t(`${plantDetailData?.lossType}`)}`}
					</div>
					<Box className='mx-4 flex flex-col gap-1 max-lg:mt-3 max-lg:rounded max-lg:bg-[#F2F2F2] max-lg:p-2 lg:mx-6'>
						...
						<div className='flex w-full flex-row gap-1 text-sm text-black lg:hidden'>
							...
							<div className='flex flex-wrap font-semibold'>{plantDetailData?.riceType[language]}</div>
						</div>

						<div className='flex w-full flex-row gap-1 text-sm text-black lg:hidden'>
							<div className='flex shrink-0'>{`${t('cropType', { ns: 'plot-monitoring' })}`}</div>:
							<div className='flex flex-wrap font-semibold'>{plantDetailData?.detailType[language]}</div>
						</div>
						<div className='flex w-full flex-row gap-1 text-sm text-black max-lg:hidden'>
