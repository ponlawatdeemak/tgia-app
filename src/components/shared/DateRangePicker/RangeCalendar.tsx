...
import { formatDate, getMonthFull } from '@/utils/date'
  ...
  const renderCalendarNavigator = (
    ...
    return (
      <div className='flex items-center justify-between' onMouseUp={(e) => e.stopPropagation()}>
        ...
        */}
        <Select id="month_list" variant="standard" size='small'
          value={`${focusedDate.getMonth()}`}
          onChange={(event: SelectChangeEvent) => changeShownDate(Number(event.target.value), 'setMonth')}>
          {monthRange.map((i) => <MenuItem key={`month_list_${i}`} value={`${i}`}>{getMonthFull(i, i18n.language)}</MenuItem>)}
        </Select>
        <Select id="year_list" variant="standard" size='small'
          value={`${focusedDate.getFullYear()}`}
          onChange={(event: SelectChangeEvent) => changeShownDate(Number(event.target.value), 'setYear')}>
          {yearRange.map((i) => <MenuItem key={`year_list_${i}`} value={`${i}`}>{(i18n.language === Language.TH) ? i + 543 : i}</MenuItem>)}
        </Select>
